...
static void UpdateSelectionFromAnchorToEnd(HWND hEdit);
...
static LRESULT CALLBACK LowLevelKbHook(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && wParam == WM_KEYDOWN && g_hMainWnd) {
        ...
        if (p->vkCode == 'V' && (GetAsyncKeyState(VK_CONTROL) & 0x8000) && (GetAsyncKeyState(VK_SHIFT) & 0x8000)) {
...
LRESULT CALLBACK EditSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg == WM_LBUTTONUP) {
        ...
        UpdateSelectionFromAnchorToEnd(hWnd);
        return r;
    }
        ...
        if (wParam == IDT_POLL_CAPTION) {
            ...
            if (text != g_lastCaptionText) {
                ...
                if (hEdit) {
                    ...
                    UpdateSelectionFromAnchorToEnd(hEdit);
